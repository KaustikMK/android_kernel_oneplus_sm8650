		...
		char __user *ubuf, size_t count, loff_t *offset)
{
	...
	if (of_device_is_compatible(tbu->dev->of_node, "qcom,qtb500")) {
		...
		for (i = 0, x = 0; i < TTQTB_Capture_Points &&
			...
			scnprintf(buf + strlen(buf), buf_len - strlen(buf),
				"Latency_%d : 0x%llx\n",
				i, (unsigned long long)ttlogs_time[x]);
			scnprintf(buf + strlen(buf), buf_len - strlen(buf),
				"Timestamp_%d : 0x%llx\n",
				i, (unsigned long long)ttlogs_time[x+1]);
			if (qtb_type == 1) {
				for (j = 0, y = 0; j < TTQTB_Regs_Per_Capture_Points &&
						y < 2*TTQTB_Regs_Per_Capture_Points; j++, y += 2) {
					scnprintf(buf + strlen(buf), buf_len - strlen(buf),
						"LogIn_%d_%d : 0x%llx\n",
						i, j, (unsigned long long)gfxttlogs[i][y]);
					scnprintf(buf + strlen(buf), buf_len - strlen(buf),
						"LogOut_%d_%d : 0x%llx\n",
						i, j, (unsigned long long)gfxttlogs[i][y+1]);
				}
			} else if (qtb_type == 2) {
				for (j = 0, y = 0; j < TTQTB_Regs_Per_Capture_Points &&
					...
					scnprintf(buf + strlen(buf), buf_len - strlen(buf),
						...
						i, j, (unsigned long long)ttlogs[i][y]);
					scnprintf(buf + strlen(buf), buf_len - strlen(buf),
						"LogIn_%d_%d_High : 0x%llx\n",
						i, j, (unsigned long long)ttlogs[i][y+1]);
					scnprintf(buf + strlen(buf), buf_len - strlen(buf),
						"LogOut_%d_%d_Low : 0x%llx\n",
						i, j, (unsigned long long)ttlogs[i][y+2]);
					scnprintf(buf + strlen(buf), buf_len - strlen(buf),
						"LogOut_%d_%d_High : 0x%llx\n",
						i, j, (unsigned long long)ttlogs[i][y+3]);
				}
		...
		char __user *ubuf, size_t count, loff_t *offset)
{
	...
	scnprintf(buf + strlen(buf), buf_len - strlen(buf),
		"Filter_TrType : 0x%llx\n", (unsigned long long)filter[i]);
	scnprintf(buf + strlen(buf), buf_len - strlen(buf),
		"Filter_AddressMin : 0x%llx\n", (unsigned long long)filter[i+1]);
	scnprintf(buf + strlen(buf), buf_len - strlen(buf),
		"Filter_AddressMax : 0x%llx\n", (unsigned long long)filter[i+2]);
